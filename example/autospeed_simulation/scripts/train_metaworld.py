USED_YAML = "train_metaworld.yaml"
...
from read_data.metaworld_eval_episode_loader import MetaWorldEvalEpisodeLoader
...
class WorkspaceIL:
    def __init__(self, cfg, rank=0, world_size=1):
        ...
        if self.cfg.eval and self.rank == 0:
            ...
            self.eval_loader = MetaWorldEvalEpisodeLoader(
                ...
                proprio_dim=self.cfg.suite.proprio_dim,
            )
        else:
    ...
    def export_eval_curve(self, agent, eval_data_paths, training_step):
        ...
        for idx, eval_data_path in enumerate(eval_data_paths):
            ...
            with torch.no_grad():
                ...
                gt_action_parts = [[] for _ in range(self.cfg.t_eval_sampling_times)]
                ...
                for start in range(0, num_frames, max_eval_batch):
                    ...
                    for t in range(self.cfg.t_eval_sampling_times):
                        gt_action_parts[t].append(gt_action_list_chunk[t].detach().cpu())

                pred_actions = torch.cat(pred_actions_parts, dim=0)
                gt_action_list = [torch.cat(parts, dim=0) for parts in gt_action_parts]
