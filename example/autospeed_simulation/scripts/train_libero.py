...
from read_data.libero_eval_episode_loader import LiberoEvalEpisodeLoader
...
class WorkspaceIL:
    def __init__(self, cfg, rank=0, world_size=1):
        ...
        if self.cfg.eval and self.rank == 0:
            ...
            self.eval_loader = LiberoEvalEpisodeLoader(
                dataset_root=self.cfg.dataloader.bc_dataset.path,
                suite=self.cfg.suite.task.suite,
    ...
    def train(self):
        ...
        while train_until_step(self.global_step):
            ...
            if self.cfg.eval and self.global_step > 0 and self.global_step % self.cfg.eval_every_steps == 0:
                if self.rank == 0:
                    print( f"start eval at training step {self.global_step}")
                    if self.cfg.suite.task.eval_data_specs is not None:
                        self.export_eval_curve(agent, self.cfg.suite.task.eval_data_specs, self.global_step)
                    else:
                        print(f"no eval_data_specs specified, skip eval")
                self._maybe_barrier()
    ...
    def export_eval_curve(self, agent, eval_data_specs, training_step):

        total_episodes = len(eval_data_specs)
        for idx, spec in enumerate(eval_data_specs):
            task_name = spec['task_name']
            demo_id = spec['demo_id']
            print(f"[Eval Progress] Processing episode {idx + 1}/{total_episodes}: {task_name} demo {demo_id}")

            episode = self.eval_loader.sample_from_spec(task_name, demo_id)

            save_path = os.path.join(self.work_dir, "eval_results", f"training_step_{training_step}")
            os.makedirs(save_path, exist_ok=True)
            save_path = os.path.join(save_path, f"{task_name}_episode_{demo_id}.hdf5")
