...
class ResnetRouter(nn.Module):
    def __init__(self, input_shape,
        ...
        if not separate_encoders:
            ...
            self.encoder = {key: ResnetEncoder(input_shape, output_dim, pretrained, frozen, remove_layer_num, no_stride, language_dim, language_fusion).to(device) for key in pixel_keys}
        self.encoder = self.encoder.to(device)

    def forward(self, pixel, pixel_key=None,lang=None):
        if self.language_fusion != "none":
            assert lang is not None, "language is required for language fusion"

        if self.separate_encoders:
            return self.encoder[pixel_key](pixel, lang)
        else:
