...
class AlohasimEvalEpisodeLoader:
    def __init__(self, eval_data_paths,
                    stats_path=None,
                    img_size=(224, 224),
                    num_queries=16,
                    speed_range=(0.5, 2.0),
    ...
    def preload_episode(self, eval_data_path):
        with h5py.File(eval_data_path, 'r') as f:
            actions = f["action"][:]
            fix_camera = f["observations/images/top"][:]
            proprioceptive = f["observations/qpos"][:]
            task_emb = f["task_emb"][:]

            num_frames = len(proprioceptive)
            print(f'frames: {num_frames}')
            if fix_camera.shape[-2] != self.img_size[0] or fix_camera.shape[-1] != self.img_size[1]:
            ...
            proprioceptive_tensor = (proprioceptive_tensor - torch.from_numpy(self.stats["min"])) / (torch.from_numpy(self.stats["max"]) - torch.from_numpy(self.stats["min"]) + 1e-5) * 2 - 1

            a_max = self.speed_range[1]
            ...
            for j in tqdm(range(num_frames)):
                ...
                chunk = extended_actions[start_idx:end_idx]
                action_chunks.append(chunk)

            action_chunks_tensor = torch.stack(action_chunks, dim=0)
            action_chunks_tensor = (action_chunks_tensor - torch.from_numpy(self.stats["min"])) / (torch.from_numpy(self.stats["max"]) - torch.from_numpy(self.stats["min"]) + 1e-5) * 2 - 1
