...
class ContinuousRectifiedFlow(DiffusionModel):
    ...
    def loss(
        ...
        action_cond: torch.Tensor,
        optimize_target: str = 'action',        # action or latent_action_linear or latent_action_mlp
        ...
        t = torch.rand((BT,), device=self.device)

        for i in range(num_gt):
            x0_gt_i = x0_group[:, i, :, :]           # [BT, chunk_size, D]
            x1_i = x1_group[:, i, :, :]

            xt_i, _, _ = self.add_noise(x0_gt_i, t=t, eps=x1_i)

            # predict velocity
            #print(xt_i.shape, t.shape, action_cond.shape)
            pred_velocity = self.model["diffusion"](xt_i, t, action_cond)
            # compute ground truth in velocity space
            gt_velocity = x0_gt_i - x1_i
            ...
            if self.criterion_type == 'noise':
                criteria.append(fm_loss)
            elif self.criterion_type == 'action':
                t_expanded = at_least_ndim(t, xt_i.dim())

                pred_x0 = xt_i + t_expanded * pred_velocity
                criteria.append((pred_x0 - x0_gt_i) ** 2)
            else:
                raise ValueError(f"Invalid criterion type: {self.criterion_type}")
        
        loss = torch.stack(loss, dim=1)
        loss = loss.mean(dim=[2, 3]) # [B*T, num_gt]

        criteria = torch.stack(criteria, dim=1)
        criteria = criteria.mean(dim=[2, 3]) # [B*T, num_gt]
    ...
    def sample(
        ...
        **kwargs,
        ...
        for i in reversed(loop_steps):
            ...
            #print(f"t_schedule[i]: {t_schedule[i]}")
        ...
        # ================= Post-processing =================
        if self.clip_pred:
            # print(f"[Flow] Before clip: xt min={xt.min().item():.4f}, max={xt.max().item():.4f}")
            xt = xt.clip(self.x_min, self.x_max)
            # print(f"[Flow] After clip:  xt min={xt.min().item():.4f}, max={xt.max().item():.4f}")
...
class ActionHead(nn.Module):
    ...
    def forward(self, action_cond, actions=None, optimize_target = 'action', init_noise=None):
        ...
        if actions is None: 
            ...
            # actions:  [B*T, num_gt , chunk_size, D]
            # metrics['loss']: [B*T, num_gt]
            ...
            if 'latent_action' in optimize_target: # latent_action_linear or latent_action_mlp
...
if __name__ == "__main__":
    action_head = ActionHead(
        ...
        sampling_steps=20,
        noise_init_varying_multispeed='same',
        criterion_type='noise',
        device='cuda',
    )
    
    action_cond = torch.randn(4, 1, 128).cuda()
    actions = torch.randn(4, 10, 10, 10).cuda()
    x_gt = action_head(action_cond)
    print(x_gt.shape)