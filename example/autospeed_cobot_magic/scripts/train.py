...
from read_data.alohareal_eval import AloharealEvalEpisodeLoader
...
def make_optimizer(cfg,agent):
    ...
    optim_ratio_head = torch.optim.AdamW(
        list(agent.ratio_head.parameters()),
        lr=cfg.lr,
        betas=(0.9, 0.95),
        eps=1e-8,
        weight_decay=1e-4,
    ...
    params_base = (
        list(agent.spatial_adapter.parameters()) +
        list(agent.proprio_projector.parameters()) +
        list(agent.language_projector.parameters()) +
...
class WorkspaceIL:
    def __init__(self, cfg, rank=0, world_size=1):
        ...
        if self.cfg.eval and self.rank == 0:
            ...
            self.eval_loader = AloharealEvalEpisodeLoader(self.cfg.suite.task.eval_data_paths,
    ...
    def export_eval_curve(self, agent, eval_data_paths, training_step):
        ...
        for idx, eval_data_path in enumerate(eval_data_paths):
            ...
            with torch.no_grad():
                (pred_actions,           #torch.Size([401, 16, 14])
                gt_action_list,          # list of torch.Size([401, 16, 14])
                ratio_head_output,       #torch.Size([401])
                update_select_speed) = agent.eval(episode, norm_stats=None,
                                                                        t_eval_sampling_times=self.cfg.t_eval_sampling_times, training_step = training_step)

                #update_select_speed torch.Size([401, 5])
            gt_action_transformed = gt_action_list[0]
    ...
    def load_snapshot(self, snapshots):
        ...
        for k, v in payload.items():
            if k == 'optimizers':
                pass
            elif k in self.__dict__: # belong to workspace
                print(f"Restoring workspace state: {k} = {v}")
                self.__dict__[k] = v
            else: # belong to model
                agent_payload[k] = v
